...
def unstringifySGF(s):
	# Input: A length 361 string
	# Output: An array of (y-coord, x-coord, color)
	sgf = []
	for y in range(19):
		for x in range(19):
			color = s[19*y + x]
...
def main(outfile, NUM_PUZZLES=861):
	""" Writes all of the sgfs into a json file. """
	allPuzzles = {}
	with open(outfile, 'w') as ofile:
		ofile.write("var allPuzzles = {\n")
		for i in range(1, NUM_PUZZLES+1):
			sgf = predictStones(i)
			sgf = json.dumps(unstringifySGF(sgf))
			ofile.write("\t")
			ofile.write(str(i))
			ofile.write(": \t")
			ofile.write(sgf)
