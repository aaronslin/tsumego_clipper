...
BoardState._initialFor = function (boardSize, handicapStones, startPosition) {
  ...
  startPosition.forEach(function (s) {
    emptyPoints[s.y * boardSize + s.x] = new _intersection2.default(s.y, s.x, s.color);
  });

  var initialState = new BoardState({
    color: startPosition.length == 0 || handicapStones > 1 ? "black" : "white",
    moveNumber: 0,
...
var VALID_GAME_OPTIONS = ["boardSize", "scoring", "handicapStones", "koRule", "komi", "_hooks", "fuzzyStonePlacement", "renderer", "freeHandicapPlacement", "startPosition"];
...
Game.prototype = {
  _configureOptions: function _configureOptions() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        ...
        startPosition = _ref$startPosition === undefined ? this._defaultStartPosition : _ref$startPosition;
    ...
    if (!((typeof startPosition === "undefined" ? "undefined" : _typeof(startPosition)) == "object" && startPosition.length >= 0)) {
      throw new Error("Start position must be an object.");
    }

    if (startPosition.length > 0 && (freeHandicapPlacement || handicapStones > 0)) {
      throw new Error("Start position can only be specified when there are no handicaps");
    }
    ...
    if (this._freeHandicapPlacement) {
      this._initialState = _boardState2.default._initialFor(boardSize, 0, startPosition);
    } else {
      this._initialState = _boardState2.default._initialFor(boardSize, handicapStones, startPosition);
    }
  },
